...
namespace memory_mapped_file
{
    ...
    void read_only_mmf::open(char const* pathname, bool map_all)
    {
        ...
        if (is_open()) close();
    #if defined(_WIN32)
        file_handle_ = ::CreateFile(reinterpret_cast<LPCWSTR>(pathname), GENERIC_READ,
        ...
        memory_mapped_file::mmf_doesnt_exist_mode doesnt_exist_mode)
    {
        ...
        file_handle_ = ::CreateFile(reinterpret_cast<LPCWSTR>(pathname), GENERIC_READ | GENERIC_WRITE,
            FILE_SHARE_READ | FILE_SHARE_WRITE, 0,
